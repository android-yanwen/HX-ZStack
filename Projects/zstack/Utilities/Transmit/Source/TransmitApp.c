      ...
      case FUN_CODE_READ_HOLDING_REGISTERS://¶Á¼Ä´æÆ÷Êý¾Ý
        {
          ...
          if(pRxFrame->ADDR==NativeAddr)//ÊÇ·ñ±¾»úµØÖ·
          {
              ...
              #if defined(GTA_WMSCB_R1)
                if(pRxFrame->pData[0] == 0x04){//ÅÐ¶ÏÊÇ·ñÊÇLEDµãÕó
                    //startTimer3();  //LEDµãÕóÐèÒª¿ªÆô¶¨Ê±Æ÷½øÐÐÉ¨Ãè
                    isScan = true;
                    u_LedsNumber = pRxFrame->pData[1];
                } else {
                    //stopTimer3();  //Èç¹û²Ù×÷µÄ²»ÊÇLEDµãÕó£¬Ò»¶¨Òª¹Ø±Õ¶¨Ê±Æ÷
                    isScan = false;
                    /*if(isScan == false){
...
*/
void initTimer3(void){
    T3CTL |= 0x08 ;   //¿ªÒç³öÖÐ¶Ï
//    T3IE = 1;   //¿ª×ÜÖÐ¶ÏºÍ T3ÖÐ¶Ï
    T3CTL|=0XE0;   //128 ·ÖÆµ,128/16000000*N=0.5S,N=65200
    T3CTL &= ~0X03;   //×Ô¶¯ÖØ×° 00£­>0xff  65200/256=254(´Î)
    T3CTL |=0X10;   //Æô¶¯
//    EA = 0;    //¿ª×ÜÖÐ¶Ï
}
...
¶¨Ê±Æ÷3ÖÐ¶Ïº¯Êý
************************************/
#pragma vector = T3_VECTOR    //¶¨Ê±Æ÷ T3
__interrupt void T3_ISR(void)
{
    IRCON = 0x00;   //ÇåÖÐ¶Ï±êÖ¾,Ò²¿ÉÓÉÓ²¼þ×Ô¶¯Íê³É
    ControlLeds(u_LedsNumber);
}
#endif